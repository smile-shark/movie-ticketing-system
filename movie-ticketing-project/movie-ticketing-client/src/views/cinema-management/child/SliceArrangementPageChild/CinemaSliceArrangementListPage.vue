<template>
  <div>
          ...
          <el-col :span="6">
            <el-form-item style="padding: 10px">
              ...
              <el-select v-model="movieId" filterable clearable>
                <el-option
                  v-for="(moive, index) in movies"
                  :key="index"
                  :label="moive.movieName"
                  :value="moive.movieId"
                ></el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :span="6">
            <el-form-item style="padding: 10px">
              ...
              <el-select v-model="screeningRoomId" filterable clearable>
                <el-option
                  v-for="(screeningRoom, index) in screeningRooms"
                  :key="index"
                  :label="screeningRoom.screeningRoomName"
                  :value="screeningRoom.screeningRoomId"
                ></el-option>
              </el-select>
            </el-form-item>
          </el-col>
    ...
    <el-row>
      <el-col :span="24">
        <el-table :data="sliceArrangements" style="width: 100%" height="72vh">
          ...
          <el-table-column label="放映时间">
            <template slot-scope="scope">
              <TruncatedText :lines="1">
                {{
                  utils.formatTimestampToYYYMMDDHHMMSS(
                    scope.row.sliceArrangementStartTime
                  )
                }}
                -
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button type="text" @click="handleEdit(scope.row)"
                >详情</el-button
              >
              <el-button type="text" @click="getOrderList(scope.row)"
                >订单列表</el-button
              >
    ...
    <el-row>
      ...
      >
        ...
        <el-button
          type="primary"
          size="small"
          @click="selectSliceArrangementBySliceArrangement(1)"
          >搜索</el-button
        >
      </el-col>
    </el-row>
    <el-dialog
      title="排片详情"
      :visible.sync="sliceArrangementDialogVisible"
      width="80%"
    >
      <el-form v-if="showSliceArrangement.movie" label-width="100px">
        ...
        <el-form-item label="放映时长：">
          {{
            utils.timeStampToMinutes(
              showSliceArrangement.sliceArrangementEndTime -
                showSliceArrangement.sliceArrangementStartTime
            )
          }}
          分钟
        </el-form-item>
        ...
        <el-form-item label="座位详情：">
          <div class="layoutCss">
            <SliceArrangementSeatLayout
              :layout="showSliceArrangement.layout"
              :is-cinema-management="true"
            ></SliceArrangementSeatLayout>
          </div>
        </el-form-item>
      </el-form>
      ...
      >
        ...
        <el-table-column label="座位">
          <template slot-scope="scope">
            <TruncatedText :lines="1">
              <span
                v-for="(seat, index) in JSON.parse(scope.row.seats)"
                :key="index"
              >
                {{ seat.x }}排{{ seat.y }}座
              </span>
        ...
        <el-table-column label="购票状态">
          <template slot-scope="scope">
            <span v-if="scope.row.votePayState == 0"> 未支付 </span>
            <span v-if="scope.row.votePayState == 1"> 已支付未使用 </span>
            <span v-if="scope.row.votePayState == 2"> 已支付已使用 </span>
            <span v-if="scope.row.votePayState == 3"> 支付超时 </span>
            <span v-if="scope.row.votePayState == 4"> 已取消支付 </span>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    selectOrderBySliceArrangementId(page = 1) {
      this.orderPage = page;
      myApi
        .selectOrderBySliceArrangementId(
          this.showsliceArrangementId,
          this.orderPage,
          this.orderSize
        )
        .then((res) => {
